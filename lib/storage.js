...
(function (root, factory) {
  ...
  // misread Task 44's fourth status exists to fix. The check runs first, but
  ...
  function deriveAiringStatus(parts) {
    if (!Array.isArray(parts) || parts.length === 0) return null;
  ...
  function withDerivedStatus(storage, title) {
    ...
    var patch = status === title.status ? null : { status: status };
  ...
  return {
    ...
    deriveAiringStatus: deriveAiringStatus,
