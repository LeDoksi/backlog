...
(function (root, factory) {
  ...
  function pushOverride(client, id, patch) {
    if (!usable(client)) return Promise.resolve(false);
    var row = { id: id, updated_at: new Date().toISOString() };
    Object.keys(patch || {}).forEach(function (k) { row[k] = patch[k]; });
    markEcho(T_OVERRIDES, id);
    return attempt('pushOverride ' + id, function () {
  ...
  function pushDelete(client, id, wasDraft) {
    ...
    return attempt('pushDelete ' + id, function () {
  ...
  function pushDraft(client, title) {
    if (!usable(client) || !title || !title.id) return Promise.resolve(false);
    markEcho(T_DRAFTS, title.id);
    return attempt('pushDraft ' + title.id, function () {
  ...
  function pushRemoveDraft(client, id) {
    ...
    return attempt('pushRemoveDraft ' + id, function () {
  ...
  function pushParts(client, id, indices) {
    if (!usable(client)) return Promise.resolve(false);
    markEcho(T_PARTS, id);
    return attempt('pushParts ' + id, function () {
