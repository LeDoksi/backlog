...
(function (root, factory) {
  ...
  var ALL_TABLES = [T_OVERRIDES, T_DRAFTS, T_PARTS];
  ...
  var WRITABLE_KEYS = [OVERRIDES_KEY, ADDED_KEY, PARTS_KEY];
  ...
  // `overrides` row (a removed title just drops out of `drafts`, and
  ...
  // Task 49: how many edits are still waiting to be sent. Read-only, and the
  // reason it lives here rather than in app.js: the queue's storage key and its
  // shape are this module's business, and a UI that reached into
  // `backlog-sync-outbox` itself would be a second place to keep right. Zero
  // when the outbox was never wired up, which is the same "nothing is queued"
  // the caller wants to hear.
  function outboxLength() { return readOutbox().length; }
  ...
  function seedLocal(client, storage, options) {
    ...
    var overrideRows, draftRows, partRows;
    try {
