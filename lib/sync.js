...
(function (root, factory) {
  ...
  var EDITABLE_OVERRIDE_FIELDS = ['title', 'category', 'year', 'genres', 'rating', 'synopsis', 'cover', 'originalTitle', 'seasonInfo', 'platforms', 'parts'];
  ...
  // A status of null means "this row exists only to carry another field" and
  // must not be merged onto the title — applyOverlay Object.assigns the
  // override straight on, so a null would blank the catalog's own status and
  // take the badge, the status filter and the status sort down with it.
  //
  // Every other field's null is the opposite: it is a real value the owner
  // set — un-rating a title, clearing an `originalTitle` — and dropping it
  // would make that the one edit that never reaches the other device. Safe to
  // carry because every entry in data.js already ships a concrete value or
  // null for each of these, so an override can only ever restore a field to
  // something it has already been.
  //
  // One generic loop for every override column, `status` included, rather
  // than a special-cased `if` per field — the field list above is the only
  // place that needs to grow when a new field becomes editable.
  function shapeOverrides(rows) {
    ...
    (rows || []).forEach(function (row) {
      ...
      OVERRIDE_FIELDS.forEach(function (field) {
        ...
        if (field === 'status' && value === null) return;
        patch[field] = value;
      });
      out[row.id] = patch;
    });
