...
- Каждая SQL-задача обязана включать шаг живой проверки через `curl` от лица второго тестового аккаунта (не только «код есть», а «RLS реально блокирует чужое пространство»).
