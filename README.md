...
Кнопка «Найти» рядом с формой быстрого добавления ищет введённое название во внешнем источнике, выбранном по категории — кино/сериалы через TMDb, игры через RAWG, аниме через Jikan (без ключа) — и показывает до 5 подходящих карточек (постер, название, год). Выбор карточки сразу создаёт черновик (`BacklogStorage.addTitle`) с уже заполненными полями (год/жанры/синопсис/обложка, платформы для игр) вместо пустого плейсхолдера — как и обычный черновик, он остаётся `draft: true` и полностью доступен для правки через «Редактировать» после сохранения. Для TMDb и RAWG нужны бесплатные ключи `TMDB_KEY`/`RAWG_KEY`, заданные константами рядом с `SUPABASE_URL` в `app.js`; без ключа или сети кнопка просто покажет короткое сообщение («не удалось найти», «добавь ключ…») и ничего не сломает — ручной ввод и обычная кнопка «+ Добавить» продолжают работать как раньше. Для аниме Jikan не отдаёт русское название/синопсис (это правило: тайтлы каталога — на русском, кроме игр), поэтому такой черновик по-прежнему помечается как требующий ручного дозаполнения.
