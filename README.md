...
- Статус просмотра, удаление и быстро добавленные черновики редактируются прямо в интерфейсе и хранятся в `localStorage` вашего браузера поверх `data.js` — четыре ключа:
...
Каталог (`data.js`) — статика, он одинаков у всех и правится только через Claude. А вот слой правок поверх него (статусы, удаления, черновики, отмеченные сезоны) — личный и общий на двоих, поэтому он живёт в Supabase и приезжает на все устройства.
