...
Вычисленный статус записывается в `backlog-overrides` тем же путём, что и обычный, — поэтому карточки, бейджи, фильтры и сортировка работают без изменений. У тайтла без `parts` всё остаётся как раньше: обычные три кнопки статуса.
